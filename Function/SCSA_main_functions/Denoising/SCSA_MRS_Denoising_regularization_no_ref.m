...
%% ######################  PARAMETERS ######################################
% Input 
% SNR_regularization: coefficient used for the optimization problem ~ 500
...
function [yscsa, h_op, Nh]=SCSA_MRS_Denoising_regularization_no_ref(SNR_regularization, ppm, yf, gm , fs , width_peaks)
% close all
...
if Plot_fig==1
      figure;
    if shif==0
        plot(ppm,y,'b','LineWidth',2.5);hold on
    end
    
    plot(ppm, yscsa+shif ,'r','LineWidth',2);hold on
    plot(ppm, y-yscsa-5,'g','LineWidth',1.5); hold off
    
    legend({'Noisy input spectrum ', 'Denoised Spectrum ', 'Residual'},'Location','northeast');
    xlabel('ppm')
    ylabel('Intensity')
